...
public class AnimationClipCopy : MonoBehaviour
{
    ...
    static void Pack(string source)
    {
        ...
        List<string> curAnimatorOverrideControllerNameArr = new List<string>();
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            else
            {
                ...
                else if (files[i].Name.ToLower().EndsWith(".overridecontroller"))
                {
                    curAnimatorOverrideControllerNameArr.Add(fileFullName);
                }
            }
        }

        s_curAnimationClipArr = new Dictionary<AnimationClip, AnimationClip>();
        ...
        for (int i = 0; i < curAnimatorOverrideControllerNameArr.Count; i++)
        {
            string path = curAnimatorOverrideControllerNameArr[i].Substring(Application.dataPath.Length - 6).Replace("\\", "/");

            AnimatorOverrideController animatorController = AssetDatabase.LoadAssetAtPath(path, typeof(AnimatorOverrideController)) as AnimatorOverrideController;
            
            for (int k = 0; k < animatorController.clips.Length; ++k)
            {
                if (animatorController.clips[k].overrideClip != null)
                {
                    Dictionary<AnimationClip, AnimationClip>.Enumerator et = s_curAnimationClipArr.GetEnumerator();
                    while (et.MoveNext())
                    {
                        if (et.Current.Key == animatorController.clips[k].overrideClip)
                        {
                            animatorController[animatorController.clips[k].originalClip.name] = et.Current.Value;
                        }
                    }
                    et.Dispose();
                }
...
#region 替换motion
    private static void AnimatorLink(string animatorControllerPath)
    {
        ...
        {
            AnimatorControllerLayer layer = animatorController.layers[i];
            CheckAndRefreshAnimatorController(animatorControllerPath, s_curAnimationClipArr, layer.stateMachine, animatorController);
        }
    }


    static void CheckAndRefreshAnimatorController(string animatorControllerPath, Dictionary<AnimationClip, AnimationClip> newClips, AnimatorStateMachine stateMachine, AnimatorController animatorController)
    {
        for (int i = 0; i < stateMachine.states.Length; i++)
        {
            ChildAnimatorState childState = stateMachine.states[i];
            ...
            if(!string.IsNullOrEmpty(motionName))
            {
                ...
                List<AnimationClip> keys = new List<AnimationClip>(newClips.Keys);

                for (int j = 0; j < keys.Count; j++)
                {
                    if (childState.state.motion == (Motion)keys[j])
                    {
                        childState.state.motion = (Motion)newClips[keys[j]];
                        Debug.Log("替换成功 animatorController:" + animatorController.name + " motionName:" + motionName);
                        s_countChange++;
                        break;
                    }
                    else if (newClips[keys[j]].name.CompareTo(motionName) == 0)
                    {
                        childState.state.motion = (Motion)newClips[keys[j]];
                        Debug.Log("替换成功 animatorController:" + animatorController.name + " motionName:" + motionName);
                        s_countChange++;
...
#region 拷贝动画clip
    ...
    private static Dictionary<AnimationClip, AnimationClip> s_curAnimationClipArr;

    private static void AnimCopy(string fbxPath, string parentPath)
    {
        Object[] objs = AssetDatabase.LoadAllAssetsAtPath(fbxPath);

        //Debug.Log("fbxPath:" + fbxPath + " parentPath:" + parentPath);
        string helloName = fbxPath.Substring(fbxPath.LastIndexOf("/") + 1);
        helloName = helloName.Replace(".FBX", "");
        Debug.Log(helloName);

        string animationPath = "";

        AnimationClipSettings setting;
        AnimationEvent[] aes = null;
        ...
        foreach (Object o in objs)
        {
            if (o.GetType() == typeof(AnimationClip) && !o.name.Contains("preview"))
            {
                ...
                newClip.name = helloName + "@" + srcClip.name;//设置新clip的名字

                //for (int i = 0; i < srcClip.events.Length; i++)
                //{
                //    newClip.AddEvent(srcClip.events[i]);
                //}
                
                s_curAnimationClipArr.Add(srcClip, newClip);
                ...
                aes = AnimationUtility.GetAnimationEvents(srcClip);//获取AnimationClipSettings

                AnimationUtility.SetAnimationClipSettings(newClip, setting);//设置新clip的AnimationClipSettings
                AnimationUtility.SetAnimationEvents(newClip, aes);//设置新clip的AnimationClipSettings

...
public class NVBoids : MonoBehaviour
{
    ...
    Vector3[] rdTargetPos, flockPos, velFlocks, tpChs;
    ...
    void FlocksMove()
    {
        ...
        //{
        //    flocksTransform[f].localPosition = Vector3.SmoothDamp(flocksTransform[f].localPosition, 
        //        flockPos[f], ref velFlocks[f], smoothChFrequency);
        //}
    ...
    void BirdsMove()
    {
        ...
        Vector3 verticalWaweCur = Vector3.right * ((verticalWawe * 0.5f) - 
            Mathf.PingPong(timeTime * 0.5f, verticalWawe));
        ...
        for (int b = 0; b < birdsNum; b++)
        {
            ...
            Vector3 tpCh = flocksTransform[curentFlock[b]].position + rdTargetPos[b]
                - birdsTransform[b].position;
            tpCh = tpCh.normalized * 3;
            //  + verticalWaweCur 
            tpCh.y = 0;
            this.tpChs[b] = tpCh;
            Quaternion rotationCur = Quaternion.LookRotation(
                Vector3.RotateTowards(birdsTransform[b].forward, tpCh, soaringCur, 0));
    ...
    {
        for (int i = 0; i < birdsTransform.Length; i++)
        {
            Gizmos.color = Color.red;
            Gizmos.DrawLine(birdsTransform[i].position, tpChs[i]);
        }
    ...
    IEnumerator BehavioralChange()
    {
        ...
        while (true)
        {
            ...
            for (int b = 0; b < birdsNum; b++)
            {
                ...
                tpChs[b] = new Vector3(lpv.x, lpv.y * fragmentedBirdsYLimit, lpv.z);

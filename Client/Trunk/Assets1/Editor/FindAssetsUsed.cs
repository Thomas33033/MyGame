...
public class ArtAssetsEx : EditorWindow{
    ...
    [MenuItem("Tools/特效贴图/自动设置贴图属性")]
    public static void SetSpritePackingTag()
    {
        ...
        foreach (Object selectObj in selectObjs)
        {
            ...
            if (selectObj.GetType() == typeof(Texture2D))
            {
                ...
                int width = (selectObj as Texture2D).width;
                int height = (selectObj as Texture2D).height;
                ...
                {
                    TextureImporterPlatformSettings ts = new TextureImporterPlatformSettings();
                    ts.name = "Android";
                    ts.overridden = true;
                    ts.maxTextureSize = width/2;
                    ts.format = TextureImporterFormat.ETC2_RGBA8;
                    textureImporter.SetPlatformTextureSettings(ts);

                    ts.name = "iPhone";
                    ts.overridden = true;
                    ts.maxTextureSize = width;
                    ts.format = TextureImporterFormat.PVRTC_RGBA4;
                    textureImporter.SetPlatformTextureSettings(ts);
                }
                    ...
                    )
                {
                    ...
                    ts.name = "Android";
                    ts.overridden = true;
                    ts.maxTextureSize = max / 2;
                    ts.format = TextureImporterFormat.ETC2_RGBA8;
                    textureImporter.SetPlatformTextureSettings(ts);

                    ts.name = "iPhone";
                    ts.overridden = true;
                    ts.maxTextureSize = max;
                    ts.format = TextureImporterFormat.PVRTC_RGBA4;
                    textureImporter.SetPlatformTextureSettings(ts);
                }
                else
                {
                    ...
                    ts.name = "Android";
                    ts.overridden = true;
                    ts.maxTextureSize = 512;
                    ts.format = TextureImporterFormat.ETC2_RGBA8;
                    textureImporter.SetPlatformTextureSettings(ts);

                    ts.name = "iPhone";
                    ts.overridden = true;
                    ts.maxTextureSize = 1024;
                    ts.format = TextureImporterFormat.PVRTC_RGBA4;
                    textureImporter.SetPlatformTextureSettings(ts);
                }
    ...
    [MenuItem("Tools/特效贴图/查找特效贴图的使用数量")]
    public static void sssss()
    {
        string srcPath = Application.dataPath + "/_Res/Bundles/Single/Arts/Images";

        List<string> allEntries = new List<string>();

        allEntries.AddRange(Directory.GetDirectories(srcPath, "*", SearchOption.AllDirectories).Select(dir => dir + "/"));
        ...
        for (int i = 0, count = allEntries.Count; i < count; ++i)
        {
            ...
            if (!ext.Equals(".meta"))
            {
                ...
                _repaint = true;
                Debug.Log(path + "  :  " + _AssetUsedData._usedPrefab.Count);
            }
        }
    }

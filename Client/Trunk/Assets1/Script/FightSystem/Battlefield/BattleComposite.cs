...
namespace Fight
{
    ...
    public class BattleComposite
    {
        ...
        public Dictionary<string, FightTeam> dicTeams;
        ...
        {
            dicTeams = new Dictionary<string, FightTeam>();
        ...
        public void Init()
        {
            for (int i = 0; i < listAllRoles.Count; i++)
            {
                ...
                {
                    FightReportFightOnBattlefield report = new FightReportFightOnBattlefield(0, fightRole.teamId, fightRole.uid, (int)fightRole.type, fightRole.id, fightRole.GetBattlefield().id, fightRole.hpMax, fightRole.mpMax, fightRole.position.ToArray());
        ...
        public bool AddRoleOnBattleField(string team, int battlefieldId, FightSeamanData crewData, int site, bool isPlayer)
        {
            BattleField battleField = dicBattleField[battlefieldId];

            FightRole fightRole = new FightRole(team, crewData.uid, GetRoleAttr(crewData), crewData.CurHp, crewData.CurMp, crewData.SkillData, crewData.Tag);
        ...
        private AttributeData GetRoleAttr(FightSeamanData info)
        {
        ...
        public void StartBattle()
        {
            ...
            listReport.Add(new FightReportGameStart(0, ""));
        ...
        private string GetPlayerRoleHpLeft()
        {
            ...
            for (int i = 0; i < listAllRoles.Count; i++)
            {
                ...
                if (fightRole.isPlayer && fightRole.type == RoleType.Fighter)
                {
                    ...
                    listHp.Add(Mathf.FloorToInt(100f * fightRole.hp / fightRole.hpMax).ToString());
                    listHp.Add(fightRole.mp.ToString());
                }
            }
        ...
        //角色死亡
        public void Die(string teamId, string id)
        {
            int num = 0;
            for (int i = 0; i < listAllRoles.Count; i++)
            {
        ...
        public void UseSkill(string roleId)
        {

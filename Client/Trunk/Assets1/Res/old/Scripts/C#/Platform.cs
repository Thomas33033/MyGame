...
public class Platform : MonoBehaviour {
    ...
    private List<Node> BuildCostNodes = new List<Node>(); //建筑建造话费的格子
    ...
    public bool CheckForBlock(Vector3 pos)
    {
        ...
        if (RefreshBulidGrid(nextBuildNode.ID))
        {
            return true;
        }
	...
	public void Build(Vector3 point, Tower tower){

		if(walkable)
        {
            if (tower.CurData.TowerType != _TowerType.Mine)
            {
                if (nextBuildNode!=null && Vector3.Distance(nextBuildNode.pos, point)<BuildManager.GetGridSize()/2)
                {
					nextBuildNode.walkable=false;
                    for (int i = 0; i < BuildCostNodes.Count; i++)
                    {
                        BuildCostNodes[i].walkable = false;
                    }
    ...
    public bool RefreshBulidGrid(int index)
    {
        ResetDefaultRes();
        BuildCostNodes.Clear();
        ...
        for (int i = 0; i < list.Count; i++)
        {
            if (list[i] >= 0 && list[i] < nodeGraph.Length)
            {
                ...
                BuildCostNodes.Add(nodeGraph[list[i]]);
            }
        }
